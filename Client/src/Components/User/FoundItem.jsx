...
function FoundItem() {
  ...
  const handleNavigateToMyItems = () => {
    navigate('/user/my-found-items');
  };

  const handleNavigateToMyLostItems = () => {
    navigate('/user/lost-items-others'); 
  };
  ...
  const validateForm = () => {
    ...
    if (!formData.description.trim()) errors.description = "Please provide some details about the item or pet.";

    if (mainCategory === 'pets') {
      if (formData.petName && !nameRegex.test(formData.petName)) errors.petName = "Name can only contain letters and spaces.";
      if (formData.itemName && !nameRegex.test(formData.itemName)) errors.itemName = "Breed can only contain letters and spaces.";
      if (!formData.color.trim()) errors.color = "Color/Markings are required to help identify the pet.";
  ...
  const handleSubmit = async (e) => {
    ...
    if (!itemImage) {
      validationErrors.image = "Please upload an image of the item or pet.";
    }
  ...
  const renderDynamicFields = () => {
    ...
    const fields = isPet ? (
      <>
        ...
        <TextField fullWidth required label="Color / Markings" name="color" value={formData.color} onChange={handleChange} placeholder="e.g., Black with a white spot on chest" InputProps={{ startAdornment: <InputAdornment position="start"><FaPaw /></InputAdornment> }} error={!!formErrors.color} helperText={formErrors.color} />
      </>
    ) : (
      <>
    ...
    return (
      <Box className="dynamic-fields-wrapper">
        ...
        <TextField fullWidth required label="Additional Details" name="description" value={formData.description} onChange={handleChange} multiline rows={4} placeholder={isPet ? "Collar details, temperament, where exactly you found it..." : "Color, any damage, custom stickers, where it was found..."} InputProps={{ startAdornment: <InputAdornment position="start" sx={{ alignItems: 'flex-start', mt: '1rem' }}><FaAlignLeft /></InputAdornment> }} error={!!formErrors.description} helperText={formErrors.description} />
      </Box>
    );
  };
      ...
      <Paper elevation={0} variant="outlined" className="add-item-paper">
        <Box sx={{
          ...
          <Button
            variant="outlined"
            startIcon={<FaSearch />}
            onClick={handleNavigateToMyLostItems}
            sx={{
              color: 'var(--clean-primary-dark)',
              borderColor: 'var(--clean-primary-dark)',
              '&:hover': {
                backgroundColor: 'rgba(25, 164, 122, 0.04)',
                borderColor: 'var(--clean-primary-color)',
              }
            }}
          >
           Lost Items
          ...
          <Button
            variant="outlined"
            startIcon={<FaListUl />}
            onClick={handleNavigateToMyItems}
            sx={{
              color: 'var(--clean-primary-dark)',
              borderColor: 'var(--clean-primary-dark)',
              '&:hover': {
