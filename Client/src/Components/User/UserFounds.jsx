...
import { FaPaw, FaLaptop, FaMobileAlt, FaDog, FaCat, FaEdit, FaExclamationTriangle, FaPlus, FaTrash, FaHandshake, FaMapMarkerAlt, FaCalendarAlt } from 'react-icons/fa';
...
function UserFounds() {
  ...
  const handleOpenModal = (actionType, item) => {
    setModalState({
      ...
      actionType,
      itemId: item._id,
      itemName: item.mainCategory === 'pets' ? (item.petName || item.itemName) : item.itemName,
    });
  };
            ...
            <Paper key={item._id} elevation={0} variant="outlined" className="item-card">
              ...
              <Box className="card-actions">
                ...
                <Button 
                  ...
                  onClick={() => handleOpenModal('markReturned', item)} 
                  disabled={updatingItemId === item._id || isArchived}
                >
      ...
      <Dialog open={modalState.open} onClose={handleCloseModal} >
        ...
        <DialogActions sx={{ p: '0 24px 20px', gap: '8px' }}>
          ...
          <Button 
            ...
            variant="contained" 
            color={modalState.actionType === 'delete' ? 'error' : 'primary'} 
            autoFocus
          >
